/-
  Trust-Lean v3.1 — Unsigned Simulation (Lifting Pattern)
  N19.4: CRITICO — stmtToMicroC_correct_uint32/64 via lifting.

  Lifting pattern (L-657/L-660/L-663):
  For call-free programs, evalMicroC_uint32 and evalMicroC agree on non-Int operations
  (comparison, logical, control flow). The only difference is wrapping at arithmetic/bitwise boundaries.

  The simulation theorem follows from the core stmtToMicroC_correct
...
3. **Translation unchanged**: stmtToMicroC is the same function — it doesn't know about wrapping

The key insight: stmtToMicroC_correct proves evalStmt → evalMicroC (unbounded).
...
- stmtToMicroC_correct: evalStmt fuel env stmt ≈ evalMicroC fuel mcEnv (stmtToMicroC stmt)
...
-- 1. stmtToMicroC_correct (Simulation.lean) gives evalStmt → evalMicroC
...
-- These three theorems together constitute the unsigned simulation guarantee.
...
/-! ## Smoke Tests: End-to-End Unsigned Evaluation -/
